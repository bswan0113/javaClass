...
public class BoardMain {
	...
	//3. 게시글 삭제
		...
		case 4: printBoard();
			break;
		case 5: System.out.println("이전 메뉴로 돌아갑니다.");
			break;
		default :
	...
	private static void runPrintMenu(int detailMenu) {
			...
			case 4:printStr("이전 메뉴로 돌아갑니다.");
				break;
				
			default:printStr("잘못된 입력입니다.");
			}
	}
	...
	private static void updateBoard() {
		...
		int index = boardList.indexOf(num);
		if(index==-1) {
			printStr("잘못된 접근입니다");return;
		}
		Board board =boardList.get(index);
		if(!board.getWirter().equals(user.getId())) {
			printStr("수정권한이 없습니다.");return;
		}
